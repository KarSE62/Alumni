<!DOCTYPE html>
<html lang="en">

<head>
  <title>ระบบค้นหาข้อมูลศิษย์เก่า</title>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no" />
  <link rel="preconnect" href="/https://fonts.googleapis.com">
  <link rel="preconnect" href="/https://fonts.gstatic.com" crossorigin>
  <link href="/https://fonts.googleapis.com/css2?family=Sriracha&display=swap" rel="stylesheet">
  <link href="/https://fonts.googleapis.com/css?family=Lato:300,400,700&display=swap" rel="stylesheet" />

  <link rel="stylesheet" href="/https://stackpath.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css" />

  <link rel="stylesheet" href="/styleLogin.css" />
</head>

<body class="img js-fullheight" style="
      background-image: url(https://i.pinimg.com/originals/80/0e/d4/800ed46000029ab99c5a3554ff5da86b.jpg);
    ">
   <section class="ftco-section">
      <div class="container">
        <div class="row justify-content-center">
          <div class="col-md-6 text-center mb-5">
            <h1 class="heading-section" style="font-size: 50px;">ระบบศิษย์เก่ามหาวิทยาลัยราชภัฏนครปฐม</h1>
          </div>
        </div>
        <div class="row justify-content-center">
          <div class="col-md-6 col-lg-4">
            <div class="login-wrap p-0">
              <div class="social d-flex text-center">
                <a href="/search" class="px-2 py-2 ml-md-1 rounded"style="font-size: 25px;"
                  ><span class="ion-logo-facebook mr-2"></span> ค้นหาข้อมูล</a
                >
              </div>
              <br />
              <div class="social d-flex text-center">
                <a href="/register" class="px-2 py-2 ml-md-1 rounded" style="font-size: 25px;"
                  ><span class="ion-logo-twitter mr-2"></span> ลงทะเบียนศิษย์เก่า</a
                >
              </div>
              <br />
              <div class="social d-flex text-center">
                <a href="./login" class="px-2 py-2 ml-md-1 rounded"style="font-size: 25px;"
                  ><span class="ion-logo-twitter mr-2"></span> เข้าสู่ระบบ</a
                >
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  <script src="jquery.min.js"></script>
  <script src="main.js"></script>
</body>

</html>